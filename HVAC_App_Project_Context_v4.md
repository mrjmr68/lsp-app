...
| **Phase 7** | 🔜 Next | PDF/email integration for invoices, or add/edit records UI |
...
app/
  ...
  customers/
    ...
    [customerId]/
      page.tsx                  — server: customer + locations (with unit counts)
      CustomerDetail.tsx        — client: info card + locations list
      [locationId]/
        page.tsx                — server: location + units (with system counts)
        LocationDetail.tsx      — client: address, access notes (yellow), units list
        [unitId]/
          page.tsx              — server: unit + all systems (full fields)
          UnitDetail.tsx        — client: system cards (read-only, all fields)
  invoices/
...
**PDF/email on approval:** Stubbed with console.log — wire in Phase 7.

---

### Customers (`/customers`) — ✅ Live
4-level drill-down, read-only browse:

| Level | URL | Content |
|---|---|---|
| 1 | `/customers` | All customers, searchable, type pills, location count |
| 2 | `/customers/[id]` | Customer info, billing, locations list with unit counts |
| 3 | `/customers/[id]/[locId]` | Address, access notes (yellow), units list with system counts |
| 4 | `/customers/[id]/[locId]/[unitId]` | All systems, full fields, subtype badges |
